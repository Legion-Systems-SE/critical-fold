...
## 6-Prime Cage Hypothesis — 7/7 Tests Pass
...
Rotating projections of the engine's node cloud reveal **eight structural
...
| Finding | Result |
|---|---|
| **Structural frequencies** | {1, 3, 5, 7, 9, 15, 27, 45} |
...
CUDA recommended for the engine but not required. The analysis tools run on CPU with NumPy + SciPy only.
...
| Script | Purpose |
|---|---|
| `engine_emergent.py` | Simulation engine (v0.4+, emergent fold, no imported zeros) |
| `tension.py` | Digit-level tension analysis (delta-2, collapse, multi-base) |
| `cage_hypothesis.py` | 6-prime cage falsification suite |
| `pole_reality_test.py` | S³ beat detune verification against IGRF-14 pole data |
| `reproduce.py` | Reproducibility verification |
| `observatory.py` | Windowed FFT + UCA beamforming |
| `radio.py` | Sonification of field resonances per rotation axis |
| `upc_test.py` | Digit-curvature resonance test suite |
| `analyze.py` | Post-run analysis dispatcher |
| `crystallograph.py` | Rotational moiré viewer |
| `visualize_3d.py` | Interactive 3D field viewer |

### Related

| Repo | Purpose |
|---|---|
| [upc-calculator](https://github.com/Legion-Systems-SE/upc-calculator) | Standalone digit-curvature resonance test with statistical controls |
